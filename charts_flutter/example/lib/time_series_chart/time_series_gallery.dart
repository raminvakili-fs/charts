...
List<charts.Series<TimeSeriesSales, DateTime>> _createRandomData() {
  ...
  dataTimesStream().listen((TimeSeriesSales tSS){
    data.add(tSS);
    print('new data added ${tSS.sales}');
  });
...
Stream<TimeSeriesSales> dataTimesStream() async* {
  final random = new Random();
  while (true) {
    await Future.delayed(Duration(seconds: 1));
    yield TimeSeriesSales(new DateTime(2017, 10, 10), random.nextInt(100));
  }
}
...
List<GalleryScaffold> buildGallery() {
  ...
  return [
    new GalleryScaffold(
      ...
      title: 'Time Series Chartz',
      subtitle: 'Simple single time series chart',
      childBuilder: () => new SimpleTimeSeriesChart(_createRandomData()),
    ),
    new GalleryScaffold(
      listTileIcon: new Icon(Icons.show_chart),
